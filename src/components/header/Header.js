...
const Header = () => {
	...
	const renderBackAndClearButtons = (
		<section className="w-full flex justify-between items-center">
			<div
				className="flex items-center flex-grow cursor-pointer hover:opacity-60 hover:underline transition-all"
				onClick={() => handleBackClick()}
			>
				<img
					src="/images/icons/arrow-back-outline.svg"
					className="mr-4 w-7"
					alt="back arrow icon"
				/>
	...
	const header = (
		<Fade>
			<header className="h-[20%] flex flex-col justify-evenly items-center">
				<h1
					className={`text-2xl md:text-4xl text-center underline transition-all ${
						selectedList === null &&
						"cursor-pointer hover:text-3xl md:hover:text-5xl header-hover"
					}`}
					onClick={openModal}
				>
					{text}
